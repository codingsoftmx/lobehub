...
description: >
  ...
  (TRPC routers, services, models) via the Agentes CLI against a local dev server.
...
Standard workflow for verifying backend changes using the Agentes CLI (`lh`) against a local dev server.
