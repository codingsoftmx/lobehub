# Agentes gateway streaming + tab-switch test harness
...
works for any Agentes streaming session.
...
- **Never identify tabs by innerText.** The active tab's text embeds a
  ` · <agent name>` suffix, so a search like `'Agentes Growth'` matches the
  active tab when the active agent happens to be Agentes Growth — and you
