...
/**
 * Agentes CLI detector
 * Tries agentes, age, ag in order; validates via --help output containing "Agentes"
 */
export const lobehubDetector: IToolDetector = {
  description: 'Agentes CLI - manage and connect to Agentes services',
  async detect(): Promise<ToolStatus> {
    ...
    for (const cmd of commands) {
      try {
        ...
        // Validate it's actually Agentes CLI by checking help output
        const { stdout: helpOut } = await execPromise(`${cmd} --help`, { timeout: 3000 });
        if (!helpOut.includes('Agentes')) continue;
