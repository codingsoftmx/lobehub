...
<Tabs>
  <Tab title="Via Skills">
    Install a web search Skill from the marketplace (Settings → Skills). Options include general web search, news search, and academic search. Some may require an API key from the search provider.
  </Tab>

  <Tab title="Via MCP Servers">
    Connect a search MCP server (Brave Search, Google Search, DuckDuckGo, etc.) in your MCP settings. This gives more control and supports custom search implementations.
  </Tab>

  <Tab title="Per-Agent Configuration">
    In Agent Settings → Skills, enable web search for that specific Agent and choose the search provider. This lets you control which Agents have web access.
  </Tab>
</Tabs>
