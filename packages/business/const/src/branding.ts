// the code below can only be modified with commercial license
// if you want to use it in the commercial usage
// please contact us for more information: hello@codingsoft.org

export const LOBE_CHAT_CLOUD = 'Agentes Cloud';

export const BRANDING_NAME = 'Agentes';
export const BRANDING_LOGO_URL = '';

export const ORG_NAME = 'CodingSoft';

export const BRANDING_URL = {
  help: undefined,
  privacy: undefined,
  subscription: undefined,
  support: undefined,
  terms: undefined,
};

export const SOCIAL_URL = {
  discord: undefined as unknown as string,
  github: 'https://github.com/codingsoftmx',
  medium: undefined as unknown as string,
  x: 'https://x.com/codingsoftmx',
  youtube: undefined as unknown as string,
};

export const FILE_URL = {
  importFromNotionGuide: 'https://hub-apac-1.lobeobjects.space/assets/notion.mp4',
};

export const BRANDING_EMAIL = {
  business: 'hello@codingsoft.org',
  support: 'soporte@codingsoft.org',
};

export const BRANDING_PROVIDER = 'codingsoft';

export const COPYRIGHT = `© ${new Date().getFullYear()} ${ORG_NAME}`;
export const COPYRIGHT_FULL = `${COPYRIGHT}. All rights reserved.`;
